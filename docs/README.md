# Documentation

This directory contains the architectural and methodological documentation
for Grocery Deal Intelligence.

## Deterministic ingestion

- [Deterministic source ingestion](deterministic-source-ingestion.md)
- [Deterministic batch source-record ingestion](deterministic-batch-ingestion.md)
- [Deterministic ingestion result set](ingestion-result-set.md)
- [Deterministic road test](deterministic-road-test.md)

## AI-assisted canonical ingestion

- [AI-assisted canonical ingestion](ai-assisted-canonical-ingestion.md)
- [Ollama integration](ollama-integration.md)
- [Real retailer AI experiment](real-retailer-ai-experiment.md)
- [Multi-retailer AI experiment](multi-retailer-ai-experiment.md)
- [Candidate rejection diagnostics](candidate-rejection-diagnostics.md)
- [Capability / canonical-schema contract audit](capability-schema-contract-audit.md)

## Product comparison

- [Evidence-grounded product comparison proposals](product-comparison-proposals.md)
- [Overridable comparison policies](comparison-policies.md)
- [Evidence-grounded normalized product attributes](normalized-product-attributes.md)
- [Evidence-grounded economic normalization](economic-normalization.md)
- [Deterministic exact price comparison](price-comparison.md)
