...
The public offer surface explicitly distinguishes `Senza PENNYCard` and `Con PENNYCard`, with separate prices and, for some products, separate unit prices.
...
Observed offer pages carry explicit validity intervals and campaign/category pages for specific flyer periods. The public source exposes overlapping campaigns/current and upcoming offers, but update cadence has not yet been measured longitudinally.

## Access and reproducibility

The source is publicly readable without authentication for the inspected offer/category pages. No authentication bypass, anti-bot bypass, or access-control circumvention was attempted. The spike did not establish a documented public API.

## Terms / legal constraint

PENNY's official `Termini di utilizzo` materially restrict reuse/reproduction of site content and preserve personal-use storage/printing semantics. This creates a material boundary for Grocery Deal Intelligence:

- technical public accessibility is not equivalent to permission for automated reusable ingestion;
- a future production retrieval path should not be implemented until the intended usage model is reviewed against those terms or explicit permission/another lawful basis is established;
...
PENNY remains technically one of the highest-value source candidates discovered. The decision is `watch` because the remaining blockers are source-contract and permission questions, not lack of useful data.
...
2. **usage/legal**: establish that the intended automated retrieval pattern is acceptable, or obtain an alternative permission/source basis.
